...
d6x=700
d6y=120
def getc(n):                                    #to get coordinates of corresponding numbers
    if(n<=10 and n>=1):
        y=525
        if(n==1):
            x=20
    ...
    if(n<=100 and n>=91):
        ...
        return x,y       
p1=0
def check1(p):                #to check snake's mouth and move to tail's no
    ...
    if(p==16):
        ...
        return p
def check2(p):                #to check ladder's bottom and climb to top
    ...
    if(p==3):
        ...
        return p
pno=0                           #player no to roll dice
p2=0
def diceprint(n):              #to print dice's number 
    if(n==1):
